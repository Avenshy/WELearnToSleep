...
while '异常' not in req.text and '出错了' not in req.text:
    ...
    for x in back:
        ...
        if('未' in x['iscomplete']):
            ...
            req = session.post(url,data={'action':'setscoinfo','cid':cid,'scoid':id,'uid':uid,'data':data,'isend':'False'},headers={'Referer':'https://welearn.sflep.com/Student/StudyCourse.aspx'})
            if('"ret":0' in req.text):
                print('Way1:Success!!!',end='')
            else:
                print('Way1:Fail!!!',end='')
            url = 'https://welearn.sflep.com/Ajax/SCO.aspx'
            req = session.post(url,data={'action':'savescoinfo160928','cid':cid,'scoid':id,'uid':uid,'progress':'100','crate':'100','status':'unknown','cstatus':'completed','trycount':'0'},headers={'Referer':'https://welearn.sflep.com/Student/StudyCourse.aspx'})
            if('"ret":0' in req.text):
                print('Way2:Success!!!')
            else:
                print('Way2:Fail!!!')
